...
class OsgcTestQuestionsRepository
{
    /**
     * The Model instance.
     * ''
    ...
    {   
        ...
        $result= TestCourseQuestion::updateOrCreate(array('id' => $data['id']), $data);
        return $result->id;
    }

    public function optionSave($id,$data)
    {
        $this->testCourseQuestionOption->where('question_id',$id)->delete(); 
        $arr['question_id']=$id;
    ...
    {
        $data=$this->model->where('question_master_id',$id)->get();
    ...
    {
        return $this->model->where('question_master_id',$setting_id)->delete();
    }
     

     public function questionDisplay($data)
     {
       return $this->model->where('question_master_id',$data['id'])->with('test_question_options')
