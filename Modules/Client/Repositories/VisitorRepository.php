...
class VisitorRepository
{
    ...
    public function getByFilters($inputs)
    {
        ...
        $query=$query->when(isset($inputs['customerId']), function($query) use($inputs){
            $query->where('customerId',$inputs['customerId']);
        });
    ...
    public function getAllMyVisitors($inputs, $client_id=null){
        ...
        ->select('id','customerId','uid','barCode','firstName','lastName','email','phone','created_at as createdAt',
        'updated_at as updatedAt','deleted_at as deletedAt','visitorTypeId','avatar','notes','visitorStatusId')
        ->orderBy('id','DESC')
        ->with([
            'visitorType'=> function($query){
            $query->select('id','type');
