...
<div id="candidate_map">
    ...
    <div class="table-responsive hide-scroll"  id="candidate-geo-mapping-tbl">
        <table class=" table table-bordered"  id="candidate-geo-mapping">
            ...
            <tbody>
                <tr>
                    <td>
                        <a href="{{ route('recruitment-job.view',$job->id)}}">{{$job->unique_key}}</a>
                    </td>
    ...
    <div id="wrapper" class="toggled siderbar-panel">
        <!-- Sidebar -->
        <div id="sidebar-wrapper" style="height:80%;">
            <ul class="sidebar-nav">
                ...
                <div id="candidate-data-left-panel">
                    ...
                    <li class="candidate_names {{'candidate_li_'.$candidate->id}}" value="{{$candidate->id}}">
                        ...
                        <a target="_blank" onmouseover="openInfoWindow({{$i+1}});" href="{{ route('recruitment.candidate.view', [$candidate->id]) }}">{{ ucfirst($candidate->name) }}</a>
                    </li>
                    @endforeach
                    @else
    ...
    <div id="view-details" class="toggled filter-details" style="display: none">
        <div id="sidebar-view-details" class="hide-vertical-scroll">
            ...
            {{ Form::open(array('url'=>route("recruitment.candidate.plot-in-map-with-customer",$job->id),'id'=>'filtering-form','method'=> 'GET','class'=>'candidate-filter-form')) }}
            ...
            <div class="form-group row ">
                <label class="col-sm-4 col-md-4 col-xs-12 float-left"> Wage Expectation</label>
           ...
           {{--  <div class="clearfix"></div>
            ...
            <div class="form-group row ">
                ...
                <div class="col-sm-8 col-md-8 col-xs-12 float-left">
                    {{ Form::select('orientation',[null=>'Please Select','Yes' => 'Yes', 'No' => 'No'],old('orientation',$request->get('orientation')),array('class'=>'form-control orientation')) }}
                </div>
            </div>
            <div class="form-group row ">
            ...
            <div class="form-group row ">
                ...
                <div class="col-sm-8 col-md-8 col-xs-12 float-left">
                    {{ Form::select('english',[null=>'Please Select', 'A - Limited - I am just learning the language.'=>'A - Limited - I am just learning the language.','B - Functional - this is my second language but I can get by.'=>'B - Functional - this is my second language but I can get by.','C - Fluent - this is my native language.'=>'C - Fluent - this is my native language.'],old('english',$request->get('english')),array('class'=>'form-control english')) }}
                </div>
            </div>
            <div class="form-group row ">
            ...
            <div class="clearfix"></div>
            <div class="form-group row ">
                <label class="col-sm-4 col-md-4 col-xs-12 float-left">Core training Completed</label>
                <div class="col-sm-8 col-md-8 col-xs-12 float-left">
                      {{ Form::select('training_completed',[null=>'Please Select', 'Yes'=>'Yes', 'No'=>'No'],old('training_completed',$request->get('training_completed')),array('class'=>'form-control training_complete')) }}
                </div>
            </div>
            <div class="clearfix"></div>
...
<script type="text/javascript">
    var markers = [];
     ...
     var training_completed_tracking_id={{ config('globals.training_completed_tracking_id') }}
     var  secondLastTrackingId=training_completed_tracking_id-1;
     $('#candidate-geo-mapping').dataTable({
      ...
      function getData(){
        ...
        $.ajax({
                type: "POST",
                url: "{{route('recruitment.candidate.candidateGeomapping-storeCandidateSessionalData')}}",
                ...
                success: function(response) {
                    url="{{ route('recruitment.candidate.candidateGeomapping-export') }}"
    ...
    function initMap() {
        ...
        if(last_update_date != null && last_update_date != "" && last_update_date != undefined) {
            ...
            $.each(candidates, function(i, item) {
                if(item.last_track) {
                    let itemDate = Date.parse(item.last_track.completed_date);
        ...
        var view_url = "{{ route('recruitment.candidate.view',[':candidate_id']) }}";
        var track_url = "{{ route('recruitment.candidate.track',[':candidate_id']) }}";
        ...
        let exception_candidate_id=[];
        $.each(candidates, function(i, item) {
            let candidate_second_last_stage=[];
            let candidate_last_stage=[];
            
            let candidate_stage = $('.candidate_stage').val();

             if(candidate_stage != "" && (item.last_track) && (item.last_track.tracking_process) && (item.last_track.process_lookups_id==training_completed_tracking_id) && (item.tracking.slice(1)[0].process_lookups_id!=secondLastTrackingId)  &&  (candidate_stage.includes((item.tracking.slice(1)[0].process_lookups_id).toString()))) {
                exception_candidate_id.push(item.last_track.candidate_id);
             ...
             if(candidate_stage != "" && (item.last_track) && (item.last_track.tracking_process) && (!candidate_stage.includes((item.last_track.tracking_process.id).toString()))) {
                if(!exception_candidate_id.includes((item.id))){
                let candidate_id = parseInt(item.last_track.candidate_id);
            ...
            // if(candidate_stage != "" && (item.last_track) && (item.last_track.tracking_process) && (item.last_track.tracking_process.id != candidate_stage)) {
            //     return;
            // }
            ...
            last_tracking_step = (null!=(item.last_track) && null!=(item.last_track.tracking_process))? item.last_track.tracking_process.display_name : '--'; //Candidate Transitioned
       ...
       track_step = '<a href="'+trackurl+'">'+((null==item.termination)?((item.last_track && item.last_track.tracking_process)?item.last_track.tracking_process.step_order+'.'+item.last_track.tracking_process.display_name:"--"):"0. Application Terminated")+'</a>';
       if(item.last_track && item.last_track.process_lookups_id==training_completed_tracking_id)
       {
        var secondLastElement=item.tracking.slice(1)[0];
        if(secondLastElement.process_lookups_id!=secondLastTrackingId)
        {
        track_step = '<a href="'+trackurl+'">'+secondLastElement.tracking_process.step_order+'.'+secondLastElement.tracking_process.display_name+'</a>';
        }
       }
       ...
       var applied_date = pad(dateobj.getDate())+"/"+pad(dateobj.getMonth()+1)+"/"+dateobj.getFullYear();
            marker = new google.maps.Marker({
                ...
                content:'<div id="content" style="min-width:700px;">' +
                            ...
                            '<div id="bodyContent">' +
                            '<label class="col-md-12 col-12 scrollable popup-div-content">   <div class="row">      <div class="col-8">         '         +'         <div class="row">            <div class="col-6 p0">Full Name</div>            <div class="col-6 p0 map-disc popup-value">'+item.name+'</div>         </div>         <div class="row">            <div class="col-6 p0">Address</div>            <div class="col-6 p0 map-disc popup-value">'+item.address+'</div>         </div>         <div class="row">            <div class="col-6 p0">City</div>            <div class="col-6 p0 map-disc popup-value">'+item.city+'</div>         </div>         <div class="row">            <div class="col-6 p0">Postal Code</div>            <div class="col-6 p0 map-disc popup-value">'+item.postal_code+'</div>         </div>	<div class="row">            <div class="col-6 p0">Email Address</div>            <div class="col-6 p0 map-disc popup-value">'+item.email+'</div>         </div>         <div class="row">            <div class="col-6 p0">Phone</div>            <div class="col-6 p0 map-disc popup-value">'+(null!=item.phone?item.phone:'--')+'</div>         </div>	<div class="row">            <div class="col-6 p0">Cellular Phone</div>            <div class="col-6 p0 map-disc popup-value">'+(null!=item.phone_cellular?item.phone_cellular:'--')+'</div>         </div>         <div class="row">            <div class="col-6 p0">Security Experience</div>            <div class="col-6 p0 map-disc popup-value">'+((null!=item.guarding_experience && null!=item.guarding_experience.years_security_experience)?item.guarding_experience.years_security_experience:0)+'</div>         </div>         <div class="row">            <div class="col-6 p0">Date Applied</div>            <div class="col-6 p0 map-disc popup-value">'+applied_date+'</div>         </div>         '         @can("view_salary_in_candidate_mapping") + '         <div class="row">            <div class="col-6">Low Wage</div>            <div class="col-6">$'+((null!=item.wage_expectation)?(Number(item.wage_expectation.wage_expectations_from).toFixed(2)):0)+'</div>         </div>	<div class="row">            <div class="col-6">High Wage</div>            <div class="col-6">$'+((null!=item.wage_expectation)?(Number(item.wage_expectation.wage_expectations_to).toFixed(2)):0)+'</div>         </div>	<div class="row">            <div class="col-6">Current Wage</div>            <div class="col-6">$'+((null!=item.wage_expectation)?(Number(item.wage_expectation.wage_last_hourly).toFixed(2)):0)+'</div>         </div>         '@endcan         +'         <div class="row">            <div class="col-6 p0">Current Employee</div>            <div class="col-6 p0 map-disc popup-value">'+((null!=item.experience)?item.experience.current_employee_commissionaries:0)+'</div>         </div>         <div class="row">            <div class="col-6 p0">Scores</div>            <div class="col-6 p0 map-disc popup-value">'+Number(item.awareness.average_score)+'</div>         </div>        <div class="row">            <div class="col-6 p0">Match Score</div>            <div class="col-6 p0 map-disc popup-value">'+((null!=item.candidate_jobs)?Number(item.candidate_jobs.rec_match_score):'--')+'</div>         </div>      <div class="row">            <div class="col-6 p0">Last Tracking Step</div>            <div class="col-6 p0 map-disc popup-value">'+track_step+'</div>         </div>      </div><div class="col-4 user-image-div"><div class="row">'+image_html+'</div></div>   </div></label>' +
                            '</div>' +
                            '</div>'
            });
...
//Openmodal id reseting after filter
       if( (candidate_stage != "")){
        let menu_tag=$('li.candidate_names');
        $.each(menu_tag, function(i, item) {
        $(item).find('a').removeAttr( "onmouseover" );
        $(item).find('a').attr('onmouseover', 'openInfoWindow('+(i+1)+');');
        });
        }
...
let activeHeight = $('.embed-responsive').height();
$('#sidebar-wrapper').height((activeHeight/100)*54);
</script>
@stop
