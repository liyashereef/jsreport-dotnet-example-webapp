...
<script>
   ...
   $('.select2').select2();
    ...
    function movetoNextTab(candidate_job_id)
    {
        ...
        var view_url = '{{ route("recruitment.applyjob.view", ":id") }}';
    ...
    {
        button_text = $form.find('button[type="submit"]').attr('class');
        if (button_text == 'yes-button')
        {
            swal({
                title: "Success",
    ...
    $(document).ready(function () {
        ...
        $('#apply-job-form').submit(function (e) {
            ...
            resize.croppie('result', {
                ...
                $.ajax({
                    ...
                    url: "{{ route('recruitment.applyjob.store') }}",
                    type: 'POST',
                    data: formData,
                    statusCode: {
        ...
        $('#screening-questions-form').submit(function (e) {
            ...
            $.ajax({
                ...
                url: "{{ route('recruitment.applyjob.storescreeningquestion') }}",
                type: 'POST',
                data: formData,
                success: function (data) {
                    if (data.success) {
        ...
        $.ajax({
                ...
                url: "{{ route('recruitment.applyjob.storepersonality') }}",
                type: 'POST',
        ...
        $.ajax({
                ...
                url: "{{ route('recruitment.applyjob.competencymatrix') }}",
                type: 'POST',
        ...
        $('body').on('click', '.backtab', function () {
            ...
            //current_active_li.nextAll().removeClass('active').removeClass('success').find('a').addClass('disabled');
            current_active_li.nextAll().removeClass('active').removeClass('success');
            $('html, body').animate({
                scrollTop: $("ul").offset().top
            }, 1000);
        });
        $('.add-previous-adresses').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('recruitment.previousaddress.add') }}",
                type: 'GET',
        ...
        $('.add-position').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('recruitment.position.add') }}",
                type: 'GET',
                data: {id: length_address},
                success: function (data) {
        ...
        $('.add-reference').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('recruitment.reference.add') }}",
                type: 'GET',
                data: {id: length_address},
                success: function (data) {
        ...
        $('.add-education').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('recruitment.education.add') }}",
                type: 'GET',
                data: {id: length_address},
                success: function (data) {
        ...
        $('.add-languages').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('recruitment.language.add') }}",
                type: 'GET',
                data: {id: length_address},
                success: function (data) {
  ...
  function removeAttachment(candidate_id,attachment_id) {
    ...
    closeOnConfirm: false
        ...
        function(){
        var url = "{{ route('recruitment.candidate.remove-attachment',[':candidate_id',':attachment_id']) }}";
   ...
   function removeDocument(candidate_id,document_id) {
    swal({
    title: "Are you sure?",
    text: "You will not be able to recover this file",
    type: "warning",
    ...
    closeOnConfirm: false
        ...
        function(){
        var url = "{{ route('recruitment.candidate.remove-document',[':candidate_id',':document_id']) }}";
            url = url.replace(':candidate_id', candidate_id);
            ...
            $.ajax({
                ...
                type: 'GET',
                success: function (data) {
                    $("#upload_file_div_"+document_id).show();
                    $("#upload_btn_div_"+document_id).show();
                    $("#attachment_remove_div_"+document_id).hide();
                    $("#attachment_name_div_"+document_id).hide();
                    $('#attachment_file_name\\.'+document_id).removeClass('success');
                },
                dataType: 'html'
            });
        swal("Deleted", "File has been deleted.", "success");
  ...
  {
      $("#ots_file").show();
      $("#ots_file_set").css('display', 'none');
      $("input[name='test_score_path']").show();
      $("input[name='test_score_path']").removeAttr('disabled');
      $("#ots_file_div input[name='test_score_path']").attr('disabled', true);
      $("input[name='test_score_file']").show();
      $("input[name='test_score_file']").removeAttr('disabled');
  ...
  function removeForceDocument(item)
  {
      debugger;
      $("#force_file").show();
      $("#force_file_set").css('display', 'none');
      $("input[name='uof_path']").show();
      $("input[name='uof_path']").removeAttr('disabled');
      $("#force_div input[name='uof_path']").attr('disabled', true);
