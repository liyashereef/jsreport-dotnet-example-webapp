...
<div class="form-group row  {{ $errors->has('skill') ? 'has-error' : '' }}" id="skill">
    ...
    <div class="col-sm-7">
    	 {{ Form::select('skill[' .$each_skill->id.']',[null=>'Please Select',"No Knowledge"=>"No Knowledge","Basic Knowledge"=>"Basic Knowledge","Good Knowledge"=>"Good Knowledge","Advanced Knowledge"=>"Advanced Knowledge","Expert Knowledge"=>"Expert Knowledge"],old('skill[' .$each_skill->id.']',isset($candidate->skills[($each_skill->id)-1]->skill_level) ? $candidate->skills[($each_skill->id)-1]->skill_level :""),array('class' => 'form-control landing-form','required'=>TRUE)) }}
