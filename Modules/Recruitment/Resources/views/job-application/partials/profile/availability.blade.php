<div class="form-group row {{ $errors->has('availability_start') ? 'has-error' : '' }}" id="availability_start">
    ...
    <div class="col-sm-7">
        {{Form::text('availability_start',old('availability_start',isset($candidate->availability->availability_start) ? $candidate->availability->availability_start :""),array('class'=>'form-control datepicker','required'=>TRUE))}}
...
<div class="form-group row {{ $errors->has('current_availability') ? 'has-error' : '' }}" id="current_availability" >
    ...
    <div class="col-sm-7">
        {{ Form::select('current_availability',[null=>'Please select from the appropriate dropdown box',"Full-Time (Around 40 hours per week)"=>"Full-Time (Around 40 hours per week)","Part-Time (Less than 40 hours per week)"=>"Part-Time (Less than 40 hours per week)"],old('current_availability',isset($candidate->availability->current_availability) ? $candidate->availability->current_availability :""),array('class' => 'form-control','required'=>TRUE,'id'=>'current_available','required'=>TRUE)) }}
        <div class="form-control-feedback">{!! $errors->first('current_availability') !!}
            <span class="help-block text-danger align-middle font-12"></span>
        </div>
    </div>
</div>
<div id="availability_explanation" class="{{ @$candidate->availability->current_availability!='Part-Time (Less than 40 hours per week)'?'hide-this-block':'' }}">
<div class="form-group row {{ $errors->has('availability_explanation') ? 'has-error' : '' }}" id="availability_explanation" style="display: {{(isset($candidate->availability->current_availability) ? $candidate->availability->current_availability :"" )=="Part-Time (Less than 40 hours per week)" }} ? "block;" : "none;" ">
    ...
    <div class="col-sm-7">
        {{Form::textarea('availability_explanation',old('availability_explanation',isset($candidate->availability->availability_explanation) ? $candidate->availability->availability_explanation :""),array('class'=>'form-control','placeholder'=>"Accepted only 500 characters", 'maxlength'=>"500",'rows'=>6))}}
...
<div class="form-group row {{ $errors->has('understand_shift_availability') ? 'has-error' : '' }}" id="understand_shift_availability">
    ...
    <div class="col-sm-7">
        {{ Form::select('understand_shift_availability',[null=>'You must select "yes" or "no" from the dropdown box',"Yes"=>"Yes","No"=>"No"],old('understand_shift_availability',isset($candidate->availability->understand_shift_availability) ? $candidate->availability->understand_shift_availability :""),array('class' => 'form-control','required'=>TRUE)) }}
        <div class="form-control-feedback">
...
<div class="form-group row {{ $errors->has('available_shift_work') ? 'has-error' : '' }}" id="available_shift_work">
    ...
    <div class="col-sm-7">
        {{ Form::select('available_shift_work',[null=>'This is a mandatory field, please enter the required information',"Yes"=>"Yes","No"=>"No"],old('available_shift_work',isset($candidate->availability->available_shift_work) ? $candidate->availability->available_shift_work :""),array('class' => 'form-control','required'=>TRUE,'id'=>'shift_work')) }}
        <div class="form-control-feedback">
...
<div class="form-group row {{ @$candidate->availability->available_shift_work!='No'?'hide-this-block':'' }}"  id="explanation_restrictions">
    ...
    <div class="col-sm-7">
        {{Form::textarea('explanation_restrictions',old('explanation_restrictions',isset($candidate->availability->explanation_restrictions) ? $candidate->availability->explanation_restrictions :""),array('class'=>' form-control','placeholder'=>"Accepted only 500 characters",'maxlength'=>"500",'rows'=>6))}}
        <div class="form-control-feedback">
