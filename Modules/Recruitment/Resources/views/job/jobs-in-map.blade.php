                ...
                <div id="candidate-data-left-panel">
                    ...
                    <li>
                       ...
                       {{--  <i style="color:@if($job->status=='approved' || $job->status=='completed' ) green @else red @endif !important;" class="fa fa-map-marker float-right location-arrow" aria-hidden="true"></i> --}}
                        <a onmouseover="openInfoWindow({{$i}});" href="{{ route('recruitment-job.view',$job->id) }}">{{ ($job->customer->client_name) }}</a>
...
<script type="text/javascript">
    ...
    function initMap() {
        ...
        $.each(jobs, function(i, item) {
            ...
            wage = '$' + item.wage.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
            ...
            marker = new google.maps.Marker({
                    ...
                    content:'<div id="content" style="min-width:500px;">' +
                            ...
                            '<div class="clearfix"></div>'+
                            '<span class="col-sm-7 col-7 float-left p0 map-label">Wage</span><span class="col-sm-5 col-5 float-left p0 map-disc">'+wage+'</span>'+
                            '<div class="clearfix"></div>'+
