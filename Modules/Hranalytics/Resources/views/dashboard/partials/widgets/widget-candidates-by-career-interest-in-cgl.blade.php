<!-- Candidates by career interest in cgl -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.9.3/Chart.min.js" charset="utf-8"></script>
<script>
    //Use function name as camalCase version of WidgetTag
    //Always use let instead of var
    widgets.define('widgetCandidatesByCareerInterestInCgl',function(payload) {
        let wc; //widget container

        function generateContent() {
            let content =
            `<div style="padding: 20px;">
                <canvas id="candidatesByCareerInterestInCgl" height="125"></canvas>
            </div>`;
        ...
        function generateGraph(chartData) {
            ...
            let ctx = wc.find('#candidatesByCareerInterestInCgl');
            let trendAnalysis = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: chartData.data.chartDetails.labels,
                    datasets: [{
                        label: 'Career Interest in CGL',
                ...
                options: {
                    ...
                    title: {
                        display: false,
                        text: 'Candidates by Career Interest In CGL'
                    },
                    scales: {
                        xAxes: [{
                            gridLines: {
                        ...
                        yAxes: [{
                            ...
                            scaleLabel: {
                                display: true,
                                labelString: 'Career Interest in CGL'
                            },
                            ticks: {
                                beginAtZero: true
                            }
