...
<div class="form-group row {{ $errors->has('first_name') ? 'has-error' : '' }}"  id="first_name">
    ...
    <div class="col-sm-7">
        {{Form::text('first_name',old('first_name',isset($candidateJob->candidate->first_name) ? $candidateJob->candidate->first_name : (isset($candidateJob->candidate->name) ? $candidateJob->candidate->name:'')),array('class'=>'form-control','required'=>TRUE,'pattern'=>"[A-Z a-z\s.\-]+"))}}
...
<div class="form-group row {{ $errors->has('last_name') ? 'has-error' : '' }}"  id="last_name">
    ...
    <div class="col-sm-7">
        {{Form::text('last_name',old('last_name',isset($candidateJob->candidate->last_name) ? $candidateJob->candidate->last_name :""),array('class'=>'form-control','required'=>TRUE,'pattern'=>"[A-Z a-z\s.\-]+"))}}
...
<div class="form-group row {{ $errors->has('dob') ? 'has-error' : '' }}" id="dob">
    ...
    <div class="col-sm-7">
        {{Form::text('dob',old('dob',isset($candidateJob->candidate->dob) ? $candidateJob->candidate->dob :""),array('class'=>'form-control datepicker','required'=>TRUE))}}
...
<div class="form-group row {{ $errors->has('phone_home') ? 'has-error' : '' }}" id="phone_home">
    <label for="phone_home" class="col-sm-5 col-form-label">Home Phone Number</label>
    <div class="col-sm-7">
        {{Form::text('phone_home',old('phone_home',isset($candidateJob->candidate->phone_home) ? $candidateJob->candidate->phone_home :""),array('class'=>'phone form-control','placeholder'=>"Please enter in the  format (XXX)XXX-XXXX. If you don't have one - enter your cell number here."))}}
        <div class="form-control-feedback">{!! $errors->first('phone_home') !!}
            <span class="help-block text-danger align-middle font-12"></span>
        </div>
    </div>
</div>
<div class="form-group row {{ $errors->has('phone_cellular') ? 'has-error' : '' }}" id="phone_cellular">
    ...
    <div class="col-sm-7">
        {{Form::text('phone_cellular',old('phone_cellular',isset($candidateJob->candidate->phone_cellular) ? $candidateJob->candidate->phone_cellular :""),array('class'=>'phone form-control','placeholder'=>"Please enter in the  format (XXX)XXX-XXXX. If you don't have one, please leave blank."))}}
...
<div class="form-group row {{ $errors->has('email') ? 'has-error' : '' }}" id="email">
    ...
    <div class="col-sm-7">
          {{Form::email('email',old('email',isset($candidateJob->candidate->email) ? $candidateJob->candidate->email :""),array('class'=>'form-control mail','required'=>TRUE,'maxlength'=>255))}}
    ...
    <div class="col-sm-3">
        <div id="candidate-image-div" class="candidate-image-div" style="width:10% !important;">
            @if(isset($candidateJob->candidate->profile_image))
                <img style="border-radius: 50%;" src="{{asset('images/uploads/') }}/{{ $candidateJob->candidate->profile_image }}" data-status="1" height="100px" width="100px" name="image" id="image-element"/>
            @else
...
<div class="form-group row {{ $errors->has('address') ? 'has-error' : '' }}" id="address">
    ...
    <div class="col-sm-7">
        {{Form::text('address',old('address',isset($candidateJob->candidate->address) ? $candidateJob->candidate->address :""),array('class'=>'form-control','placeholder'=>"If you have an apartment number, please enter the unit number first, then address",'required'=>TRUE,'maxlength'=>255))}}
...
<div class="form-group row {{ $errors->has('city') ? 'has-error' : '' }}" id="city">
    ...
    <div class="col-sm-7">
        {{Form::text('city',old('city',isset($candidateJob->candidate->city) ? $candidateJob->candidate->city :""),array('class'=>'form-control','placeholder'=>"Please enter the city where you currently reside",'required'=>TRUE,'maxlength'=>255))}}
...
<div class="form-group row {{ $errors->has('postal_code') ? 'has-error' : '' }}" id="postal_code">
    ...
    <div class="col-sm-7">
        {{Form::text('postal_code',old('postal_code',isset($candidateJob->candidate->postal_code) ? $candidateJob->candidate->postal_code :""),array('class'=>'form-control postal-code','placeholder'=>"Please enter the postal code where you currently reside (6 characters)",'pattern'=>"(.){6,6}",'required'=>TRUE,'maxlength'=>6))}}
...
@foreach($candidateJob->candidate->addresses as $addres)
@include('hranalytics::job-application.partials.profile.previous-address', array('addres' => $addres))
@endforeach
@endif
