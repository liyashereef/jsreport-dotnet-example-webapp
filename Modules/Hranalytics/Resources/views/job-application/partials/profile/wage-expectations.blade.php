...
<div class="form-group row {{ $errors->has('wage_expectations_from') ? 'has-error' : '' }}" id="wage_expectations_from">
    <label for="wage_expectations_from" class="col-sm-5 col-form-label">From ($/Hour)</label>
    <label class="col-sm-1 col-form-label">$</label>
    <div class="col-sm-6">
        {{Form::text('wage_expectations_from',old('wage_expectations_from',isset($candidateJob->candidate->wageexpectation->wage_expectations_from) ? number_format($candidateJob->candidate->wageexpectation->wage_expectations_from, 2,'.','') :""),array('class'=>'form-control','placeholder'=>"Please state your minimum wage expectations",/*'step'=>"0.01",'min'=>"14.0",*/'required'=>TRUE))}}
...
<div class="form-group row {{ $errors->has('wage_last_hourly') ? 'has-error' : '' }}" id="wage_last_hourly">
    ...
    <label class="col-sm-1 col-form-label">$</label>
    <div class="col-sm-6">
        {{Form::text('wage_last_hourly',old('wage_last_hourly',isset($candidateJob->candidate->wageexpectation->wage_last_hourly) ? number_format($candidateJob->candidate->wageexpectation->wage_last_hourly, 2,'.','') :""),array('class'=>'form-control','placeholder'=>"Please enter your most recent hourly wage",/*'step'=>"0.01",'min'=>"14.0",*/'required'=>TRUE))}}
    ...
    <div class="col-sm-5">
        {{ Form::select('wage_last_hours_per_week',
                            ...
                            old('wage_last_hours_per_week',
                                isset($candidateJob->candidate->wageexpectation->wage_last_hours_per_week) ?
                                explode("." ,$candidateJob->candidate->wageexpectation->wage_last_hours_per_week)[0] :""
                                ),
                            array(
...
<div class="form-group row {{ $errors->has('current_paystub') ? 'has-error' : '' }}" id="current_paystub">
    ...
    <div class="col-sm-7">
        {{ Form::select('current_paystub',[null=>'Please select from the dropdown list',"Yes"=>"Yes","No"=>"No"],old('current_paystub',isset($candidateJob->candidate->wageexpectation->current_paystub) ? $candidateJob->candidate->wageexpectation->current_paystub :""),array('class' => 'form-control','required'=>TRUE)) }}
    ...
    <div class="col-sm-7">
        {{ Form::select('wage_last_provider', $lookups['security_provider'],
            old('wage_last_provider',
                isset($candidateJob->candidate->wageexpectation->wage_last_provider) ? $candidateJob->candidate->wageexpectation->wage_last_provider :""),
                array(
...
<div class="form-group row {{ !isset($candidateJob) || (isset($candidateJob->candidate->wageexpectation->wage_last_provider) && $candidateJob->candidate->wageexpectation->wageprovider->security_provider!='Other')?'hide-this-block':'' }}" id="wage_last_provider_other">
    ...
    <div class="col-sm-7">
        {{ Form::text('wage_last_provider_other',
            ...
            isset($candidateJob->candidate->wageexpectation->wage_last_provider_other) 
            ? $candidateJob->candidate->wageexpectation->wage_last_provider_other 
            :""
...
@php
if(isset($candidateJob->candidate->wageexpectation->wage_last_provider))
{
...
<div class="form-group row {{ $errors->has('security_provider_strengths') ? 'has-error' : '' }}" id="security_provider_strengths">
    <label for="security_provider_strengths" class="col-sm-5 col-form-label">{{(isset($candidateJob->candidate->wageexpectation->wage_last_provider))?'What were  the strengths of'. ' '.$security_provider_name. '?':' '}}</label>
    <div class="col-sm-7">
        {{Form::textarea('security_provider_strengths',old('security_provider_strengths',isset($candidateJob->candidate->wageexpectation->security_provider_strengths) ? $candidateJob->candidate->wageexpectation->security_provider_strengths :""),array('class'=>'form-control security_provider_details','placeholder'=>"Accepted only 1000 characters", 'maxlength'=>"1000",'required'=>TRUE))}}
...
<div class="form-group row {{ $errors->has('security_provider_notes') ? 'has-error' : '' }}" id="security_provider_notes">
    <label for="security_provider_notes" class="col-sm-5 col-form-label">{{((isset($candidateJob->candidate->wageexpectation->wage_last_provider))?'What do you hope to get from Commissionaires that you feel '. ' '.$security_provider_name. ' was not providing?':' ')}}</label>
    <div class="col-sm-7">
        {{Form::textarea('security_provider_notes',old('security_provider_notes',isset($candidateJob->candidate->wageexpectation->security_provider_notes) ? $candidateJob->candidate->wageexpectation->security_provider_notes :""),array('class'=>'form-control security_provider_details','placeholder'=>"Accepted only 1000 characters", 'maxlength'=>"1000",'required'=>TRUE))}}
...
<div class="form-group row {{ $errors->has('rate_experience') ? 'has-error' : '' }}" id="rate_experience">
    <label for="rate_experience" class="col-sm-5 col-form-label">{{(isset($candidateJob->candidate->wageexpectation->wage_last_provider))?'How would you rate your experience at'. ' '.$security_provider_name. '?':' '}}</label>
    <div class="col-sm-7">
          {{ Form::select('rate_experience',[null=>'Please select from the dropdown list.']+$lookups['experience_ratings'],old('last_role_held',isset($candidateJob->candidate->wageexpectation->rate_experience) ? $candidateJob->candidate->wageexpectation->rate_experience :""),array('class' => 'form-control security_provider_details','required'=>TRUE)) }}
...
<div class="form-group row {{ $errors->has('last_role_held') ? 'has-error' : '' }}" id="last_role_held">
    ...
    <div class="col-sm-7">
        {{ Form::select('last_role_held',[null=>'Please select from the dropdown list. Leave blank if this does not apply to you']+$lookups['positions_lookups']+[0=>'Other'],old('last_role_held',isset($candidateJob->candidate->wageexpectation->last_role_held) ? $candidateJob->candidate->wageexpectation->last_role_held :""),array('class' => 'form-control select2','required'=>TRUE)) }}
...
<div class="form-group row {{ $errors->has('explanation_wage_expectation') ? 'has-error' : '' }}" id="explanation_wage_expectation">
    ...
    <div class="col-sm-7">
        {{Form::textarea('explanation_wage_expectation',old('explanation_wage_expectation',isset($candidateJob->candidate->wageexpectation->explanation_wage_expectation) ? $candidateJob->candidate->wageexpectation->explanation_wage_expectation :""),array('class'=>'form-control','placeholder'=>"Accepted only 500 characters", 'maxlength'=>"500",'rows'=>6,'required'=>TRUE))}}
