...
<div class="form-group row">
    ...
    <div class="col-sm-7">
        {{Form::text('date_today',(isset($candidateJob->submitted_date) ? date('F j, Y',strtotime($candidateJob->submitted_date)) : date('F j, Y')),array('readonly'=>true,'class'=>'form-control'))}}
...
<div class="form-group row">
    ...
    <div class="col-sm-7">
        {{ Form::select('orientation',[null=>'Please Select','1' => 'Yes', '0' => 'No'] ,old('orientation',isset($candidateJob->candidate->referalAvailibility->orientation) ? $candidateJob->candidate->referalAvailibility->orientation :""),array('class' => 'form-control', 'required' => 'true')) }}
        <div class="form-control-feedback"></div>
    </div>
</div>
...
<div class="form-group row  {{ $errors->has('job_post_finding') ? 'has-error' : '' }}" id="job_post_finding">
    ...
    <div class="col-sm-7">
        {{ Form::select(
                'job_post_finding',
                [null=>'Please Select'] + $session_obj['job_post_finding'],
                old(
                    'job_post_finding',
                    isset($candidateJob->candidate->referalAvailibility->job_post_finding) ? $candidateJob->candidate->referalAvailibility->job_post_finding :""),
                    array(
...
<div id="job_post_referral" class="{{ @$candidateJob->candidate->referalAvailibility->job_post_finding!=3 ?'hide-this-block':'' }}">
<div class="form-group row {{ $errors->has('sponser_email') ? 'has-error' : '' }}" id="sponser_email">
    ...
    <div class="col-sm-7">
        {{Form::email('sponser_email',(isset($candidateJob->candidate->referalAvailibility) ?$candidateJob->candidate->referalAvailibility->sponser_email:'') ,array('class'=>'form-control','placeholder'=>'Enter email address of your sponsor'))}}
        <div class="form-control-feedback">
...
@include('hranalytics::job-application.partials.profile.candidate-referral')
...
<div class="form-group row {{ $errors->has('fit_assessment_why_apply_for_this_job') ? 'has-error' : '' }}" id="fit_assessment_why_apply_for_this_job">
    ...
    <div class="col-sm-7">
        ...
        {{ Form::select('brand_awareness_id',[null=>'Please Select'] + $session_obj['brand_awareness'],old('brand_awareness_id',isset($candidateJob->brand_awareness_id) ? $candidateJob->brand_awareness_id :""),array('class' => 'form-control', 'required' => 'true')) }}
...
<div class="form-group row {{ $errors->has('security_awareness_id') ? 'has-error' : '' }}" id="security_awareness_id">
    ...
    <div class="col-sm-7">
        {{ Form::select('security_awareness_id',[null=>'Please Select'] + $session_obj['security_awareness'],old('security_awareness_id',isset($candidateJob->security_awareness_id) ? $candidateJob->security_awareness_id :""),array('class' => 'form-control', 'required' => 'true')) }}
...
<div class="form-group row {{ $errors->has('candidate_commissionaires_understandings_id') ? 'has-error' : '' }}" id="candidate_commissionaires_understandings_id">
    ...
    <div class="col-sm-7">
        ...
        if(isset($candidateJob->candidate->comissionaires_understanding)){
            foreach($candidateJob->candidate->comissionaires_understanding as $val){
            ...
            @foreach($lookups['commissionaires_understanding'] as $val=>$understanding)
                <li class="nav-item">
                    <label class="radio-button-label">
                        <input type="radio" name="candidate_commissionaires_understandings_id[]" value={{$val}}  @if (isset($candidateJob->candidate->comissionaires_understanding)) {{ in_array($val, $selected_understandings)  ? 'checked="checked"' : '' }} @endif></input>{{++$i}}{{' - '.$understanding}}
                    </label>
                </li>
            @endforeach
        </ul>
...
@include('hranalytics::job-application.partials.profile.candidate-basic-info')
...
@include('hranalytics::job-application.partials.profile.licensing-and-security-guarding-experience')
...
@include('hranalytics::job-application.partials.profile.positions-held-in-past')

<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Compensation</label>
@include('hranalytics::job-application.partials.profile.wage-expectations')

<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Availability</label>
@include('hranalytics::job-application.partials.profile.availability')

<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Security Clearance</label>
@include('hranalytics::job-application.partials.profile.security-clearance')

<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Proximity to Client Site</label>
@include('hranalytics::job-application.partials.profile.proximity')
...
@if ( isset($candidateJob) && Auth::user()!=null)
@php $count=count($candidateJob->candidate->employment_history) @endphp
@foreach($candidateJob->candidate->employment_history as $key=>$history)
@include('hranalytics::job-application.partials.profile.employement-history', array('history' => $history,'key'=>$key,'maximum'=>$count))
@endforeach
@else
@include('hranalytics::job-application.partials.profile.employement-history')
@endif
...
@if ( isset($candidateJob) && Auth::user()!=null)
@php  $count=count($candidateJob->candidate->references) @endphp
@foreach($candidateJob->candidate->references as $key=>$references)
@include('hranalytics::job-application.partials.profile.references', array('references' => $references,'key'=>$key,'maximum'=>$count))
@endforeach
@else
@include('hranalytics::job-application.partials.profile.references')
@endif
...
@if ( isset($candidateJob) && Auth::user()!=null)
 @php  $count=count($candidateJob->candidate->educations) @endphp
@foreach($candidateJob->candidate->educations as $key=>$educations)
@include('hranalytics::job-application.partials.profile.education', array('educations' => $educations,'key'=> $key,'maximum'=>$count))
@endforeach
@else
@include('hranalytics::job-application.partials.profile.education')
@endif
...
<div class="form-group row">
    ...
    <div class="col-sm-7">
        <a href="javascript:void(0);" class="add-education">
            <i class="fa fa-plus" aria-hidden="true"></i> Add Education
        </a>
    </div>
</div>
<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Languages</label>
@include('hranalytics::job-application.partials.profile.languages')
...
@include('hranalytics::job-application.partials.profile.skills')

<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Technical Summary </label>
@include('hranalytics::job-application.partials.profile.technical_summary')

<label class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">Commissionaires Experience</label>
@include('hranalytics::job-application.partials.profile.experiences')
@include('hranalytics::job-application.partials.profile.indigenous_status')
@include('hranalytics::job-application.partials.profile.misc')
<script>
