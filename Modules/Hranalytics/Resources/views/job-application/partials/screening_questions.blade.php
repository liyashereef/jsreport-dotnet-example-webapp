...
<div class="form-group row"  {{ $errors->has('answer.'.$screening_question->id) ? 'has-error' : '' }}" id="answer.{{$screening_question->id}}">
    ...
    <div class="col-sm-7">
        ...
        {{Form::textarea('answer[' .$screening_question->id. ']',
            old('answer',isset($candidateJob->candidate->screening_questions[$j]->answer) 
            ? $candidateJob->candidate->screening_questions[$j]->answer :""),array('class'=>'form-control','placeholder'=>"", 'maxlength'=>"500",'rows'=>6,'required'=>TRUE))}}
        <!-- Set Scores -->
        <input type="hidden" name="_sc[{{$screening_question->id}}]"
        value="{{isset($candidateJob->candidate->screening_questions[$j])
            ?$candidateJob->candidate->screening_questions[$j]->score
            :''}}"/>
