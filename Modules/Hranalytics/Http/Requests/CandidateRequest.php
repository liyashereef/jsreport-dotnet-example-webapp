...
class CandidateRequest extends Request
{
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            'job_post_finding' => 'bail|required|exists:job_post_finding_lookups,id',
            'sponser_email' => 'bail|nullable|required_if:job_post_finding,==,3|email',
            ...
            'wage_expectations_from' => 'bail|required|numeric',
            ...
            'wage_last_hours_per_week' => 'bail|required|numeric|max:100|min:0',
            ...
            'disposition_granted' => 'required_if:criminal_convicted,==,Yes',
        ...
        if ($use_of_force == "Yes") {
            if (isset($force_doc_id)) {
                $rule_force = [
                    'force_certification' => 'bail|required',
                    'force_expiry' => 'bail|required|date_format:"Y-m-d"|after:today',
                    'force_file' => 'bail|required'
                ];
            } else {
                $rule_force = [
                    'force_certification' => 'bail|required',
                    'force_expiry' => 'bail|required|date_format:"Y-m-d"|after:today',
                    'force_file' => 'bail|required|mimetypes:application/pdf,image/jpeg,image/png,image/webp,image/gif,image/bmp,image/svg+xml|max:3072',
                ];
            }
        ...
        if ($criminal_convicted == "Yes") {
            $rule_criminal = [
                ...
                'disposition_granted' => 'bail|required|max:100',
        ...
        if ($position_availibility==1) {
            $rule_position_availibility = [
                'floater_hours' => 'bail|required|numeric|max:168',

            ];
            $rules = array_merge($rules, $rule_position_availibility);
        }
    ...
    {
        return [
            'floater_hours.max'=> 'Hours should be less than or equal to 168',
            'floater_hours.required'=> 'Please enter the hours',
            ...
            'job_post_finding.required' => 'Please select where you found out about this job posting',
            'job_post_finding.exists' => 'Invalid data selected',
            'sponser_email.required' => 'Please enter email address of person who referred you',
            'sponser_email.email' => 'Please enter a valid email',
            'guard_licence.required' => 'Please select the licensing information',
            'use_of_force.required' => 'Please select the use of force information',
            'force_certification.required' => 'Force certification is required since use of force chosen is Yes',
            'force_expiry.required' => 'Expiry date of  force certification is required since use of force chosen is Yes',
            'force_expiry.date_format' => 'Please enter the date in Y-m-d format',
            'force_file.required'=>'Please upload file',
            'force_file.mimetypes'=>'Please upload file of type image or pdf',
            'force_file.max'=>'File too large to upload',
            ...
            'disposition_granted.required' => 'Enter the disposition granted',
