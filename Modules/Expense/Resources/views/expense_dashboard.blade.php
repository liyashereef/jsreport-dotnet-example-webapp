...
<div class="content-component px-3 pt-3 content2 ">
    ...
    <div class="row mainlink-component card-view-section mb-2  position-relative">
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12  px-1 pb-2">
            <div  data-id="1"  class="card card-expense">
                <div class="card-body d-flex align-items-center py-2">
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12  px-1 pb-2">
            <div data-id="3" class="card card-expense">
                <div class="card-body d-flex align-items-center py-2">
                    <div class="d-flex pr-4">
                        <span class="d-flex flex-column  pr-2">
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12  px-1 pb-2">
            <div data-id="4" class="card card-expense">
                <div class="card-body d-flex align-items-center py-2">
                    <span class="d-flex flex-column  pr-2">
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12  px-1 pb-2">
            <div data-id="5" class="card card-expense">
                <div class="card-body d-flex align-items-center py-2">
                    <span class="d-flex flex-column  pr-2">
            ...
            <div class="col-md-6">
                <nav>
                    ...
                    @if(auth()->user()->can('view_all_expense_claim') || auth()->user()->can('view_allocated_expense_claim'))
                        <a class="nav-item nav-link expense active" href="#">Expense Claim</a>
                    @endif
                    @if(auth()->user()->can('view_all_mileage_claim') || auth()->user()->can('view_allocated_mileage_claim'))
                        <a class="nav-item nav-link expense" id="mileageClaim">Mileage Claim</a>
                    @endif
                    </div>
                </nav>
            </div>
                ...
                <div class="col-md-12 row border p-2 m-0">
                    ...
                    <div class="col-md-4 text-align-right align-self-end p-0">
                        @if(auth()->user()->can('view_all_expense_claim') || auth()->user()->can('view_allocated_expense_claim'))
                            ...
                            <label for="viewmyexpense">View my expense claim</label>
                        @endif
                    </div>
                </div>
            </div>
...
<table class="table table-bordered" id="expense-table">
    <thead>
        <tr>
            ...
            <th width="25%"  class="sorting">Description</th>
            <th class="sorting">Total Amount</th>
            {{--  <th class="sorting">GL Code</th>--}}
            <th class="sorting">Receipt</th>
          {{--     <th class="sorting">If No,Reason Why</th>--}}
            {{-- <th class="sorting">View Attached Receipts</th>
                    <th class="sorting">Total Amount to Reimburse(Including MST)</th> --}}
            {{-- <th class="sorting">MST Recoverable</th>
                    <th class="sorting">Billable to Project</th> --}}
            <th class="sorting">Billable</th>
            {{--    <th class="sorting">Project Name (Number)</th>--}}
            <th  class="sorting">Status</th>
            <th width="10%"  class="sorting">Submitted Date</th>
            <th width="10%"  class="sorting">Created Date</th>
            {{-- <th  class="sorting">View Client Signoff On Expense</th> --}}
            <th>Actions</th>
        </tr>
    </thead>
</table>
...
<script>
    ...
    $(function(){
         ...
         $(".employee-filter").change(function(){
             ...
             var url = "{{ route('expense-claims.list',[':viewmyexpense',':startdate',':enddate',':status',':employee']) }}";
        ...
        sessionStorage.setItem("expense_start_date", $('#startdate').val());
        sessionStorage.setItem("expense_end_date", $('#enddate').val());
            count = 0 ;
            $('#startdate').datepicker({
                format: 'yyyy-mm-dd',
                showRightIcon: false,
                change: function(e) {
                 if(sessionStorage.getItem("expense_start_date") != $('#startdate').val()){
                  changeDateFilter();
                 }
                },
            });
             $('#enddate').datepicker({
                ...
                showRightIcon: false,
                change: function(e) {
                  if(sessionStorage.getItem("expense_end_date") != $('#enddate').val()){
                    changeDateFilter();
                  }
                },
            });
            ...
            $('#mileageClaim').on('click', function() {
                $(this).attr("href", "{{ route('mileage-dashboard.index') }}/"+viewMyExpense);
            });

        var url = "{{ route('expense-claims.list',[':viewmyexpense',':startdate',':enddate']) }}";
        var viewmyexpense = $('#viewmyexpense').is(":checked") ? 1 : 0;
        var startdate = $('#startdate').val();
        var enddate = $('#enddate').val();
        url = url.replace(':viewmyexpense', viewmyexpense);
        ...
        try{
            ...
            columns: [
...

                {
                    data: null,
                        ...
                        render: function (o) {
                         ...
                         link = '<a title="Download" target="_blank" href="' + view_url + '"><i class="fas fa-download"></i></a><br>';
                ...
                {data: 'ordering_created_at', name: 'ordering_created_at',visible:false},
                {
                    ...
                    sortable: false,
                    render: function (row) {
                        var url = '{{ route("expense-claims-single",'') }}';
                        actions = '';
...
function changeDateFilter(){
    ...
    function updateCount(startdate,enddate, employee){
        ...
        var url = "{{ route('expense-claims.getCounts',[':viewmyexpense',':startdate',':enddate', ':employee']) }}";
        url = url.replace(':viewmyexpense', viewmyexpense);
        url = url.replace(':startdate', startdate);
        url = url.replace(':enddate', enddate);
        url = url.replace(':employee', employee);
    ...
    $('.card-expense').on('click', function(e){
