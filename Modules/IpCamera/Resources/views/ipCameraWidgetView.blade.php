...
<script type="text/javascript">
    ...
    //Route configruation
    const routes = {
        getTabs: "{{route('ip-camera.tabs')}}",
        getDashboardDetails: "{{route('ip-camera.tab-details')}}",
        //Widgets API's
    }
    //Dashboard script
    const dashboard = {
...
$(document).on({
    ...
    $(document).on("click",".cameraid",function(e){
        e.preventDefault();
        let id=$(this).attr("attr-id");
        let name=$(this).attr("attr-name");
        $.ajax({
            type: "post",
            url: "{{route("ip_camera.cameratoken")}}",
            data: {cameraId:id},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function (response) {
                ...
                sessionStorage.setItem("camera_id", id);
                $("#ipcamera-genral-tab-view").attr("src", url);
                $("#heading-span-0-cam-0").html(name)
    ...
    $(document).ready(function () {
        if(sessionStorage.length > 0){
            var url = sessionStorage.getItem('camera_url');
            setTimeout(() => {
                $("#ipcamera-genral-tab-view").attr("src", url);
            }, 10000);
        }
