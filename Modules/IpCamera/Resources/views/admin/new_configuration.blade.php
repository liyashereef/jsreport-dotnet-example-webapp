...
<h3 class="orange">{{(isset($tabDetails) && isset($tabDetails['id']))? 'Edit':'New'}} Ip Camera Widget</h3>
@stop
...
<script type="text/javascript">
    ...
    function save_tab_configurations() {
        ...
        var module_fields_mapping_count = document.querySelectorAll('#hidden_array_section .module_hidden_input').length;
        console.log(total_drop_box_count);
        ...
        $.ajax({
            type: "POST",
            url: "{{route('ip-camera-dashboard.saveTabDetails')}}",
            data: {
                'tab_id': tab_id,
            ...
            success: function(response) {
                if(response.status === "success") {
                    if(tab_id !== "") {
                        ...
                        function () {
                            let url = "{{ route('ip-camera-dashboard.new-configuration-index')}}";
                            window.location = url;
                        });
                    }
                }else {
