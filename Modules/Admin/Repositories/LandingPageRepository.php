...
use Modules\Admin\Models\LandingPageTabDetail;
use Modules\Admin\Models\LandingPageWidgetField;
...
class LandingPageRepository
{
    ...
    protected $landingPageWidgetLayoutModel, $landingPageModuleWidget, $landingPageWidgetLayoutDetail, $shiftModuleField, $landingPageTab, $landingPageTabDetail, $landingPageWidgetField, $shiftModuleRepository;
    ...
    public function __construct(
        ...
        LandingPageTab $landingPageTab,
        LandingPageTabDetail $landingPageTabDetail,
        LandingPageWidgetField $landingPageWidgetField,
        ...
        $this->landingPageTabDetail = $landingPageTabDetail;
        $this->landingPageWidgetField = $landingPageWidgetField;
    ...
    public function getShiftModuleFieldsById($moduleId)
    {
        return $this->shiftModuleField->withTrashed()
    ...
    public function saveTab($tabName, $customerId, $layoutId, $seqNo, $defaultStructure, $tabId = '')
    {
        ...
        if (!empty($tabId)) {
            $existingData = $this->landingPageTab->find($tabId);
            if (!empty($existingData)) {
                $data['customer_id'] = $existingData->customer_id;
            }

            $tab = $this->landingPageTab->updateOrCreate(array('id' => $tabId), $data);
        } else {
            $tab = $this->landingPageTab->updateOrCreate($data);
        }
        return $tab;
    }

    public function validateTabName($tabName, $customerId, $tabId)
    {
        $tabName = ucwords(strtolower($tabName));
        $qry = $this->landingPageTab->where('tab_name', $tabName)
            ->where('deleted_at', null);
        ...
        $count = $qry->where('customer_id', $customerId)->count();
        if ($count > 0) {
            return 1;
        }
        return 0;
        ...
        if (count($resultArray) > 0) {
            ...
            foreach ($resultArray as $result) {
                foreach ($result as $data) {
                    ...
                    $tabDetails = $this->landingPageTabDetail->updateOrCreate($tabDetail);
                    ...
                    if (!empty($tabDetails)) {
                        ...
                        $dataArr = [
                            'landing_page_tab_detail_id' => $tabDetails->id,
                        ...
                        $fieldDetails = $this->landingPageWidgetField->updateOrCreate($dataArr);
                    }
                }
            }
        }
    ...
    public function getTabsByCustomerIdOnly($customerid)
    {
        return $this->landingPageTab->with(['tabDetails'])
    ...
    {
        return $this->landingPageTabDetail->with(['landingPageModuleWidgetName', 'widgetFields'])->where('landing_page_tab_id', $tabId)->get();
    }

    public function getTabDetailsByTabId($tabId)
    {
        return $this->landingPageTab->with(['tabDetails', 'widgetLayouts'])->find($tabId);
    }

    public function createFieldsListByModuleParams($modelName, $moduleId)
    {
        $fieldsList = [];
        if ($modelName == LandingPageRepository::SHIFT_MODULE_TYPE) {
            $fieldsList = $this->bindScheduleWithWidgetParams();

            $data = $this->getShiftModuleFieldsById($moduleId);
            if (!empty($data)) {
                foreach ($data as $ky => $value) {
                    if ($value->dropdown_id) {
                        $fieldsList[] = [
                            ...
                            'model_name' => LandingPageRepository::SHIFT_MODULE_TYPE,
                            'default_sort' => false,
    ...
    {
        return $this->landingPageTabDetail->find($Id);
    }

    public function removeTabDetailsByTab($tabId)
    {
        $qry = $this->landingPageTabDetail->where('landing_page_tab_id', $tabId);
    ...
    {
        return $this->landingPageWidgetField->whereIn('landing_page_tab_detail_id', $tabDetailIdArray)->delete();
    }

    public function updateTabActiveStatus($tabid, $status)
    {
        return $this->landingPageTab
            ->where('id', $tabid)
            ->update(['active' => $status]);
    }

    public function fetchLatestTabsByLayoutGroupping($customerids)
    {
        $qry = $this->landingPageTab
            ->with(['tabDetails'])
            ->where('active', true);
        if (count($customerids) != 1) {
            $qry = $qry->where('default_tab_structure', true);
        } elseif ((!(\Auth::user()->hasAnyPermission(['admin', 'super_admin']))) || (count($customerids) == 1)) {
            $qry = $qry->whereIn('customer_id', $customerids);
        }
        $qry = $qry->orderBy('seq_no', 'asc');
        return $qry;
    }

    public function fetchModuleNameByType($moduleId, $moduleType)
    {
        if ($moduleType == LandingPageRepository::SHIFT_MODULE_TYPE) {
            $moduleData = $this->shiftModuleRepository->get($moduleId);
            return (!empty($moduleData)) ? $moduleData->module_name : '';
        } else {
    ...
    public function fetchModuleLayoutHierarchyByTab($tabId, $divWidth, $divHeight)
    {
        ...
        foreach ($moduleLayoutHierarchy->tabDetails as $tabDetail) {
            ...
            $widgetDetails = $tabDetail->landingPageModuleWidgetName;
            $layoutDetails = $tabDetail->landingPageWidgetLayoutDetail;
            ...
            $moduleResultArr[$tabDetail->landing_page_widget_layout_detail_id]['category'] = ($tabDetail->landing_page_module_widget_type == 'ShiftModule') ? 'LandingWidget' : $widgetDetails->widget_category;
    ...
    public function deleteTab($tabId)
    {
        ...
        return $this->landingPageTab->find($tabId)->delete();
    }
    ...
    {
        $result = [];
        $systemFields = $this->landingPageWidgetField
            ->whereHas('landingPageWidgetTabDetail', function ($query) use ($tabId, $moduleId) {
                return $query->where('landing_page_tab_id', $tabId)
    ...
    public function addOrRemoveTabAccordingToPermission($tabId)
    {
        ...
        foreach ($moduleLayoutHierarchy->tabDetails as $tabDetail) {
            ...
            } elseif (Auth::user()->can("view_shift_module_widgets")) {
                $havePermission++;
            }
        }
