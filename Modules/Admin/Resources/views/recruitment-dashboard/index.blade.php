...
<h3 class="orange">Recruitment Widget Configurations</h3>
@stop
...
<script type="text/javascript">
    function open_new_configuration() {
        window.location = "{{route('recruitment_dashboard.new_configuration_new')}}";
    }

    $(function(){
            $.ajax({
            type: "get",
            url : "{{route('recruitment_dashboard.getRecruitingAnalyticsDetails')}}",
            headers: {
    ...
    function edit_tab(tab_id) {
        $(".close").trigger('click');
        let url = "{{ route('recruitment_dashboard.new_configuration_new',['tab_id' => ''])}}" + tab_id + '';
        window.open(url);
    }
...
function delete_tab(tab_id) {
    swal({
            ...
            $.ajax({
                type: "POST",
                url: "{{route('recruitment_dashboard.removeTab')}}",
                data: {'tabid': tab_id},
                headers: {
...
function edit_activeTab(prevTabId) {
   ...
   $.ajax({
            type: "POST",
            url: "{{route('recruitment_dashboard.saveTabActiveStatus')}}",
            data: {
