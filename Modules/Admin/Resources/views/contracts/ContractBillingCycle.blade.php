...
<h1>Contract Billing Frequency</h1>
@stop @section('content')
<div class="add-new" data-title="Add New Contract Billing Frequency">Add
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Billing Frequency</h4>
            </div>
            {{ Form::open(array('url'=>'#','id'=>'contract-billing-cycle-form','class'=>'form-horizontal', 'method'=> 'POST')) }}
            <input type="hidden" name="id" id="id" />
            <div class="modal-body">
                    <div id="form-errors"></div>
                 <div class="form-group row" id="billingcycletitle">

                    <label for="billingcycletitle" class="col-sm-3 control-label">Billing Frequency <span class="mandatory">*</span></label>
                    <div class="col-sm-9">
                        
                        <input type="text" class="form-control has-error" name="billingcycletitle" id="billingcycletitle" placeholder="Billing Frequency" value="" />
...
<script type="text/javascript">
     $.fn.dataTable.ext.errMode = 'throw';
        ...
        var table = $('#reason-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('view-billing-cycle.show') }}",
            headers: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function (o) {
                        ...
                        actions += '<a href="#" class="edit editreason {{Config::get('globals.editFontIcon')}}" data-id=' + o.id + ' data-change-title=\'' + o.title + '\' ></a>'
                        
                        @can('lookup-remove-entries')
                            actions += '<a href="#" class="delete deletereason  {{Config::get('globals.deleteFontIcon')}}" data-id=' + o.id + ' data-change-title=\'' + o.title + '\' ></a>';
                            @endcan
                        return actions;
                        },
                }
        ...
        $(document).on("submit","#contract-billing-cycle-form",function(e){
            ...
            var id = $("#id").val();
            var url = "{{route('save-billing-cycle')}}";
            var message = '';
            $form = $('#contract-billing-cycle-form');
            if (id < 1) {
                url = "{{route('save-billing-cycle')}}";
                message = 'Rate change period has been created successfully';
            } else {
                url = "{{route('update-billing-cycle')}}";
            ...
            swal({
                ...
                function () {
                    if(true){
                        $.ajax({
                type: "post",
                url: "{{route('delete-billing-cycle')}}",
                data: {billingcycleid:id},
