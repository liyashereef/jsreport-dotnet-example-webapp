<input type="hidden" name="element_id[]" class="cls-element-id" value="{{$template_form['id'] ?? ""}}"/>
<input type="hidden" name="length[]" class="cls-length-id" value="{{$template_form['length'] ?? "1"}}"/>
    <td aria-controls="position-table" class="cls-slno">{{$template_form['position'] ?? "1"}}<input type="hidden" name="position[]" value="{{$template_form['position'] ?? "1"}}"/></td>
    ...
    <td aria-controls="position-table" class="cls-parent-question">
        <div class="form-group parent_question dropdown" id="parent_question_{{isset($key)?($key):"0"}}">
            <select class="form-control cls-parent-question-select" name="parent_question[]" id="" data-prev="{{$template_form['parent_position'] ?? ""}}" required @if(isset($template_form['show_if_yes']) ) readonly @endif>
                <option>NA</option>
    ...
    <td aria-controls="position-table" style="width:500px;" class="cls-question-text">
        <div class="form-group question_text" id="question_text_{{isset($key)?($key):"0"}}">
            <textarea name="question_text[]" id="" cols="50" rows="4" placeholder="Question" required @if(isset($template_form['answer_type_id']) && $template_form['answer_type_id'] == 4) readonly @endif>{{$template_form['question_text']  ?? ''}}</textarea>
        <span class="help-block"></span>
        </div>
    </td>
        ...
        <div class="cls-score" @if(isset($template_form['answer_type_id']) && $template_form['answer_type_id'] !=  1) style="display: none;" @endif>
            <label class="custom-control custom-radio cls-yes-value">
             <div class="form-group yes_value" id="yes_value_{{isset($key)?($key):"0"}}">
                ...
                <span class="custom-control-description">Yes</span>
                <span><input type="text" name="yes_value[]"  min=0 class="width-small-adjust" value="{{$template_form['score_yes']  ?? ""}}" @if(!isset($template_form['answer_type_id'])||(isset($template_form['answer_type_id']) && $template_form['answer_type_id'] ==  1)) required @endif></span>
                 <span class="help-block"></span>
             </div>
            </label>
            ...
            <label class="custom-control custom-radio cls-no-value">
                ...
                <span class="custom-control-indicator"></span>
                <span class="custom-control-description">No</span>
                <span><input type="text" name="no_value[]" min=0 class="width-small-adjust" value="{{$template_form['score_no']  ?? ""}}" @if(!isset($template_form['answer_type_id'])||(isset($template_form['answer_type_id']) && $template_form['answer_type_id'] ==  1)) required @endif></span>
                 <span class="help-block"></span>
                    </div>
            </label>
         </div>
