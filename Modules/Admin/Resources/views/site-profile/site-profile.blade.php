...
@section('title', config('app.name', 'Laravel').'-Site Profile')
@section('content_header')
<h1>Assign Site Profile</h1>
@stop
@section('content')
<div class="add-new" data-title="Add New Site Profile">Add
    <span class="add-new-label">New</span>
</div>
<table class="table table-bordered" id="site-profile-table">
    <thead>
        <tr>
            <th>#</th>
            <th>Site Profile Name</th>
            <th>Site Number</th>
            <th>Site Name</th>
            <th>Created Date</th>
...
<div class="modal fade" id="myModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            {{ Form::open(array('url'=>'#','id'=>'site_profile_form','class'=>'form-horizontal', 'method'=> 'POST')) }}
            {{ Form::hidden('id', null) }}
            <div class="modal-body">
                ...
                <div class="form-group" id="profile_name">
                    <label for="profile_name" class="col-sm-3 control-label">Enter Site Profile Name</label>
                    <div class="col-sm-9">
                        {{ Form::text('profile_name',null,array('class' => 'form-control', 'Placeholder'=>'Site Profile Name', 'required'=>TRUE)) }}
                        <small class="help-block"></small>
                    </div>
                </div>
                {{--<div class="form-group" id="site_number">
                    <label for="site_number" class="col-sm-3 control-label">Enter Site Number</label>
                    <div class="col-sm-9">
                        {{ Form::text('site_number',null,array('class' => 'form-control','required'=>TRUE)) }}
                        <small class="help-block"></small>
                    </div>
                </div>--}}
                <div class="form-group" id="customer_id">
                    <label for="customer_id" class="col-sm-3 control-label">Choose Customer<span class="mandatory">*</span></label>
                    <div class="col-sm-9">
                     {{Form::select('customer_id',[''=>'Please Select']+$customer_list,null, ['class' => 'form-control'])}}
                     <small class="help-block"></small>
                 </div>
             </div>
             <div class="modal-sub-title">
            ...
            <div  class="form-group" id="mandatory_course.0">
                <label for="mandatory_course" class="col-sm-3 control-label">Mandatory Courses<span class="mandatory">*</span></label>
...
<script>
    $(function () {
      ...
      try{
        var table = $('#site-profile-table').DataTable({
           dom: 'lfrtBip',
           bprocessing: false,
           buttons: [
           {
    ...
    /* Site Profile Save - Start */
    $('#site_profile_form').submit(function (e) {
        e.preventDefault();
        if($('#site_profile_form input[name="id"]').val()){
            var message = 'Site profile has been updated successfully';
        }else{
            var message = 'Site profile has been created successfully';
        }
        formSubmit($('#site_profile_form'), "{{ route('site-profile.store') }}", table, e, message);
    });
    /* Site Profile Save - End */

    /* Site Profile Edit - Start */
    $("#site-profile-table").on("click", ".edit", function (e) {
        var id = $(this).data('id');
        var url = '{{ route("site-profile.single",":id") }}';
        var url = url.replace(':id', id);
        $('#site_profile_form').find('.form-group').removeClass('has-error').find('.help-block').text('');
        $.ajax({
            url: url,
            type: 'GET',
            success: function (data) {
                if (data) {
                 ...
                 $("#myModal").modal();
                 $('#myModal .modal-title').text("Edit Site Profile: "+ data.profile_name)
             } else {
                alert(data);
            }
        },
    ...
    $('#site-profile-table').on('click', '.delete', function (e) {
        var id = $(this).data('id');
        var base_url = "{{ route('site-profile.destroy',':id') }}";
        var url = base_url.replace(':id', id);
        var message = 'Site profile has been deleted successfully';
