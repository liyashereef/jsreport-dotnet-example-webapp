...
@section('title', 'Template Allocation')
@section('content_header')
<h1>Visitor Log - Template Allocation</h1>
@stop
@section('content')
<fieldset>
    <div id="filter">
    ...
    $(function () {
        ...
        try{
            var table = $('#allocation-table').DataTable({
                ...
                ajax:{
                    url: '{{ route('template-allocation.list') }}', // Change this URL to where your json data comes from
                ...
                'columnDefs': [
                    {
                        ...
                        return '<input type="checkbox" id="tmp_id" name="template_id" value="' + $('<div/>').text(data).html() + '">';
                        }
                    },
                ...
                columns: [
                    ...
                    },
                    {
                        data: null,
                        render: function (o){
                            return (o.client_name!=null) ? '<a class="unallocate btn fa fa-minus-square fa-lg" data-id=' + o.id + '>' + '</a>':'';
                        }
                    }
                ]
            });
       ...
       $("#allocation-table_wrapper").addClass("datatoolbar");
        ...
        $('.allocate-submit-btn').on('click', function (e) {
            ...
            template_id = $("#allocation-table input[name=template_id]:checked").length;
            console.log(customer_id,template_id);
            if (Number(customer_id) > 0  && Number(template_id) > 0){
                template_ids = [];
                $("#allocation-table input[name=template_id]:checked").each(function () { template_ids.push($(this).val()); });
                template_ids = (JSON.stringify(template_ids));
                $.ajax({
                url: '{{route('template-allocation.allocate')}}',
                        method: 'POST',
                        data:  {'customer_id': customer_id, 'template_ids':template_ids},
                        success: function (data) {
                            if (data.success) {
                                swal("Allocated", "Template has been allocated", "success");
                                table.ajax.reload();
                            } else {
                                swal("Alert", "Template(s) already allocated to this customer", "warning");
                            }
                        },
                        error: function (xhr, textStatus, thrownError) {
                        //alert(xhr.status);
                ...
                var concat_str = "and ";
                if(Number(customer_id) <= 0 && Number(template_id) <= 0){
                    msg_str = msg_str + "customer " + concat_str + "template";
                } else if(Number(customer_id) <= 0){
                    msg_str = msg_str + "customer ";
                }
                else{
                    msg_str = msg_str + "template"
                }
        ...
        $("#allocation-table").on("click", ".unallocate", function (e) {
            ...
            swal({
                ...
                if (unalloc){
                    $.ajax({
                    url: '{{route('template-allocation.unallocate')}}',
                            type: 'POST',
                            data:  {'customer_id': customer_id, 'template_id':id},
                            success: function (data) {
                            if (data.success) {
                                swal("Unallocated", "Template has been unallocated", "success");
                                ...
                                swal("Alert", "No template allocated", "warning");
                            }
                            },
                            error: function (xhr, textStatus, thrownError) {
                                //alert(xhr.status);
        ...
        $('#customer-filter').on('change', function () {
            if($('#customer-filter').prop('checked') && $('#customer_id').val() != 0){
                ...
                table.ajax.url('template-allocation/list/'+$customer_id).load();
            }else{
                table.ajax.url('template-allocation/list').load();
            }
        });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function(key,value){
                        options += '<option value="'+value.id+'">'+value.project_number+'-'+value.client_name+'</option>'
                    });
                    $("#customer_id").html('');
