    ...
    $(function () {
        ...
        try {
            var table = $('#service-table').DataTable({
                ...
                fixedHeader: true,
                ajax: {
                    "url": "{{ route('uniform-scheduling-custom-question-option-list.list') }}",
                ...
                {
                data: 'uniform_scheduling_custom_question_option_allocation.[, ].uniform_scheduling_custom_question_option.custom_question_option',
                name: 'uniform_scheduling_custom_question_option_allocation.[0].uniform_scheduling_custom_question_option.custom_question_option',
                sortable: false
        ...
        $('#custom-question-form').submit(function (e) {
            e.preventDefault();
            var $form = $(this);
            url = "{{ route('uniform-scheduling-custom-question.store') }}";
        ...
        $("#service-table").on("click", ".edit", function (e) {
            ...
            $('.new-fields').remove();
            id = $(this).data('id');
            var url = '{{ route("uniform-scheduling-custom-question.edit",":id") }}';
                ...
                success: function (data) {
                    if (data) {
                        ...
                        $('#myModal input[name="is_required"]').prop( "checked", data.is_required );
                          ...
                          $.each(data.uniform_scheduling_custom_question_option_allocation, function( key, value ) {
                            if(key>=1)
                            {
                                addRow();
                            }
                            $('#myModal #option_'+key).val(value.uniform_scheduling_custom_question_option.custom_question_option)
                             $('#myModal #optionid_'+key).val(value.uniform_scheduling_custom_question_option.id)
        ...
        $('#service-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('uniform-scheduling-custom-question.destroy',':id') }}";
