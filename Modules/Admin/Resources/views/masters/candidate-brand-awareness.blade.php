...
@section('title', 'Candidate Brand Awareness')

@section('content_header')
<h1>Candidate Brand Awareness</h1>
@stop

@section('content')
<div id="message"></div>
<div class="add-new" data-title="Add New Brand Awareness">Add
    <span class="add-new-label">New</span>
</div>
<table class="table table-bordered" id="brand-awareness-table">
    <thead>
        <tr>
            <th>#</th>
...
<script>
    $(function () {
        ...
        try{
        var table = $('#brand-awareness-table').DataTable({
            bProcessing: false,
            responsive: true,
            dom: 'lfrtBip',
             buttons: [
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('candidate-brand-awareness.list') }}",
            headers: {
        ...
        $('#brand-awareness-form').submit(function (e) {
            e.preventDefault();
            if($('#brand-awareness-form input[name="id"]').val()){
                var message = 'Brand awareness has been updated successfully';
            }else{
                var message = 'Brand awareness has been created successfully';
            }
            formSubmit($('#brand-awareness-form'), "{{ route('candidate-brand-awareness.store') }}", table, e, message);
        });
        /* Save Request Type - End*/

         /* Editing Request Type - Start */
        $("#brand-awareness-table").on("click", ".edit", function (e) {
            var id = $(this).data('id');
            var url = '{{ route("candidate-brand-awareness.single",":id") }}';
            var url = url.replace(':id', id);
            $('#brand-awareness-form').find('.form-group').removeClass('has-error').find('.help-block').text('');
            $.ajax({
                url: url,
                type: 'GET',
                data: "id=" + id,
        ...
        $('#brand-awareness-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('candidate-brand-awareness.destroy',':id') }}";
            var url = base_url.replace(':id', id);
            var message = 'Brand awareness has been deleted successfully';
