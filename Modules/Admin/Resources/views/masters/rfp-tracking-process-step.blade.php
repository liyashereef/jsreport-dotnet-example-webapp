...
<h1> RFP Tracking Process Steps</h1>
@stop

@section('content')
<div class="add-new" data-title="Add New RFP Tracking Process Step">Add <span class="add-new-label">New</span></div>
...
<script>
    ...
    $(function () {
        ...
        var table = $('#tracking-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('rfp-tracking-process.list') }}",
            headers: {
        ...
        $('#tracking-form').submit(function (e) {
            ...
            formSubmit($('#tracking-form'), "{{ route('rfp-tracking-process.store') }}", table, e, message);
        });
        $("#tracking-table").on("click", ".edit", function (e) {
            var id = $(this).data('id');
            var url = '{{ route("rfp-tracking-process.single",":id") }}';
        ...
        $('#tracking-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('rfp-tracking-process.destroy',':id') }}";
