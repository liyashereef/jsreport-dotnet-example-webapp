...
<h1>Employee Whistleblower Category</h1>
@stop
@section('content')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row" id="roles">
                    <label for="roles" class="col-sm-3 control-label">Category</label>
                    <div class="col-sm-9">
                        {{ Form::text('roles',null,array('class'=>'form-control','required'=>true)) }}
                        <small class="help-block"></small>
                    </div>
                </div>
                <div class="form-group row" id="shortname">
...
<script>
    $(function () {
        ...
        var table = $('#type-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('employee-whistleblower-category.list') }}",
            headers: {
        ...
        $('#type-form').submit(function (e) {
            ...
            if($('#type-form input[name="id"]').val()){
                var message = 'Employee whistleblower category has been updated successfully';
            }else{
                var message = 'Whistleblower category has been created successfully';
            }
            formSubmit($('#type-form'), "{{ route('employee-whistleblower-category.store') }}", table, e, message);
        });


        $("#type-table").on("click", ".edit", function (e) {
            var id = $(this).data('id');
            var url = '{{ route("employee-whistleblower-category.single",":id") }}';
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        ...
                        $('#myModal input[name="roles"]').val(data.roles)
                        $('#myModal input[name="shortname"]').val(data.shortname)
                        $("#myModal").modal();
                        $('#myModal .modal-title').text("Edit Category: "+ data.roles)
                    } else {
                        alert(data);
                    }
                },
        ...
        $('#type-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('employee-whistleblower-category.destroy',':id') }}";
            var url = base_url.replace(':id', id);
            var message = 'Employee whistleblower category has been deleted successfully';
