...
<script>
    $(function () {
        ...
        var table = $('#type-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('smart-phone-type.list') }}",
            headers: {
        ...
        $('#type-form').submit(function (e) {
            e.preventDefault();
             if($('#type-form input[name="id"]').val()){
                var message = 'Smart phone type has been updated successfully';
            }else{
                var message = 'Smart phone type has been created successfully';
            }
            formSubmit($('#type-form'), "{{ route('smart-phone-type.store') }}", table, e, message);
        });


        $("#type-table").on("click", ".edit", function (e) {
            var id = $(this).data('id');
            var url = '{{ route("smart-phone-type.single",":id") }}';
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        ...
                        $("#myModal").modal();
                        $('#myModal .modal-title').text("Edit Position: "+ data.type)
                    } else {
                        alert(data);
                    }
                },
        ...
        $('#type-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('smart-phone-type.destroy',':id') }}";
            var url = base_url.replace(':id', id);
            var message = 'Smart phone type has been deleted successfully';
