...
<h1>Emergency Contact Relations</h1>
@stop
@section('content')
<div class="add-new" data-title="Add New Relation">Add
...
<script>
    $(function () {
        ...
        var table = $('#type-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('user-emergency-contact-relation.list') }}",
            headers: {
        ...
        $('#type-form').submit(function (e) {
            e.preventDefault();
             if($('#type-form input[name="id"]').val()){
                var message = 'Relation has been updated successfully';
            }else{
                var message = 'Relation has been created successfully';
            }
            formSubmit($('#type-form'), "{{ route('user-emergency-contact-relation.store') }}", table, e, message);
        });


        $("#type-table").on("click", ".edit", function (e) {
            var id = $(this).data('id');
            var url = '{{ route("user-emergency-contact-relation.single",":id") }}';
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        ...
                        $('#myModal input[name="relations"]').val(data.relations)
                        $('#myModal input[name="apogee_code"]').val(data.apogee_code)
                        $("#myModal").modal();
                        $('#myModal .modal-title').text("Edit Relation: "+ data.relations)
                    } else {
                        alert(data);
                    }
                },
        ...
        $('#type-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('user-emergency-contact-relation.destroy',':id') }}";
            var url = base_url.replace(':id', id);
            var message = 'Relation has been deleted successfully';
