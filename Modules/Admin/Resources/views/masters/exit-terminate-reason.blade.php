...
<h1>Exit Termination Reasons</h1>
@stop
@section('content')
...
<script>
    $(function () {
        ...
        var table = $('#type-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('exit-termination-reason.list') }}",
            headers: {
        ...
        $('#type-form').submit(function (e) {
            ...
            formSubmit($('#type-form'), "{{ route('exit-termination-reason.store') }}", table, e, message);
        });
        ...
        $("#type-table").on("click", ".edit", function (e) {
            var id = $(this).data('id');
            var url = '{{ route("exit-termination-reason.single",":id") }}';
        ...
        $('#type-table').on('click', '.delete', function (e) {
            var id = $(this).data('id');
            var base_url = "{{ route('exit-termination-reason.destroy',':id') }}";
