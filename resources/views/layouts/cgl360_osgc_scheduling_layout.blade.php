...
<head>
    <title>@yield('title', config('app.name', 'CGL360 OSGC') ) </title>
    <meta charset="utf-8">
...
<body>
        <nav class="navbar navbar-expand-md head-bg">
            @if(\Auth::guard('osgcuser')->user())
                <div class="row align-items-center">
                    <div class="col-md-3">
                        <a href="{{ url('osgc/home') }}"><img src="{{asset('images/logo.png') }}"></a>
                    </div>
                    <div class="col-md-9">
                    ...
                    <ul class="nav navbar-nav ml-auto mt-1">
                        <div class="dropdown">
                                ...
                                <div class="d-flex flex-row-reverse align-items-center">
                                    ...
                                    <div class="info">
                                        ...
                                        <p class="hedder-name user-name" style="">
                                            Hello {{ ucfirst(\Auth::guard('osgcuser')->user()->first_name) }} {{ ucfirst(\Auth::guard('osgcuser')->user()->last_name) }}
                                        </p>
                            ...
                            <ul class="dropdown-menu dropdown-content">
                                <li onclick="window.location=$(this).find('a').prop('href');" ><a href="{{ url('osgc/change-password') }}"> Change Password </a></li>
                                <li style="cursor:pointer" onclick="if(confirm('Are you sure to logout?')){ event.preventDefault(); document.getElementById('logout-form').submit(); }"><a>Logout</a>
                                    <form id="logout-form" action="{{ url('osgc/logout') }}" method="POST"
                                        style="display: none;">
                                        {{ csrf_field() }}
                                    </form></li>
                            </ul>
